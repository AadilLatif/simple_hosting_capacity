...
class HostingCapacity:
    ...
    def add_pv_system(self, bus_name, nodes, kv_base):
        ...
        self.pv_size_increment_kw = self.pv_size_increment_kw_lv if kv_base < 1.0 else self.pv_size_increment_kw_hv
        ...
        while not self.voltage_violation and not self.thermal_violation:
            if i == 0:
                cmd(f"New PVSystem.pv_{bus_name} bus={bus_name}.{nodes_str}  phases={len(nodes)} kv={kv} kva={pv_kva} pf=1.0 irradiance=1.0 Pmpp={pv_kva * self.ac_dc_ratio}")
            else:
                cmd(f"Edit PVSystem.pv_{bus_name} bus={bus_name}.{nodes_str}  phases={len(nodes)} kv={kv} kva={pv_kva} pf=1.0 irradiance=1.0 Pmpp={pv_kva * self.ac_dc_ratio}")
            cmd(f"solve")
        ...
        cmd(f"Edit PVSystem.pv_{bus_name} bus={bus_name}.{nodes_str}  phases={len(nodes)} kv={kv} kva={pv_kva} pf=1.0 irradiance=1.0 Pmpp={pv_kva * self.ac_dc_ratio} enabled=false")
