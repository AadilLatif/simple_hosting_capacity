...
@click.option('-l', '--pv-size-increment-lv', default=100.0, help='PV size increment in kVA (for low voltage buses).')
@click.option('-h', '--pv-size-increment-hv', default=200.0, help='PV size increment in kVA (for high voltage buses).')
@click.option('-f', '--feeeder-head-voltage', default=1, help='Voltage at the head of the feeder in pu.')
@click.option('-p', '--load-power-factor', default=1, help='Load power factor.')
@click.option('-a', '--ac-dc-ratio', default=1, help='AC / DC ratio for the PV systems.')
@click.option('-m', '--load-mult', default=1, help='Load multiplier.')
@click.option('-t', '--load-model-type', default=1, help='Load model type (see OpenDSS documentation).')
...
@click.argument("model")
@click.option('-p', '--hosting-capacity-csv', help='PV hosting capacity data (csv file).')
@click.option('-l', '--line-width-dynamic', default=True, help='Number of greetings.')
@click.option('-v', '--voltage-distance-file', default="voltage_distance_plot.html", help='Path to voltage distance plot export.')
@click.option('-h', '--heatmap-file', default="heatmap_plot.html", help='Path to heatmap plot export.')
@click.command()
def build_plots(
    model,
    hosting_capacity_csv,
    line_width_dynamic,
    voltage_distance_file,
    heatmap_file
